...
## 🔴 13c. O aplicativo ainda diz "caneta" para quem usa frasco ou comprimido

**Esta é uma dívida assumida, e não um descuido.** Ela nasceu de uma
decisão consciente, e o raciocínio fica aqui para quem for pagá-la.

O aplicativo passou a conhecer medicamento que não é caneta: semaglutida
oral em comprimido, e semaglutida ou tirzepatida manipuladas em frasco ou
seringa preenchida. O cadastro pergunta a forma quando o catálogo não
sabe, e a folha de registrar já se adapta — para comprimido ela nem chama
aquilo de aplicação.

Mas a palavra "caneta" está escrita em **mais de trinta arquivos**, e a
maioria deles não foi tocada. Quem escolher comprimido vai encontrar
"caneta" em telas que estas mudanças não alcançaram.

**Por que não segurei a pergunta até a varredura estar pronta:** sem a
forma existir, a varredura não teria como saber o que escrever no lugar.
A ordem certa é esta — primeiro o conceito, depois a palavra.

### Onde a dívida está

Contado por arquivo, incluindo comentários de código (que também precisam
ser relidos, porque vários explicam decisões em termos de caneta):

| arquivo | menções | o que é |
|---|---|---|
| `src/logic/derive.ts` | 27 | `canetaAtual`, `penStock`, comentários |
| `src/logic/meds.ts` | 22 | quase tudo comentário sobre validade |
| `src/app/aplicacoes.tsx` | 14 | copy de tela — **a maior dívida de texto visível** |
| `src/app/(tabs)/index.tsx` | 9 | copy da Home |
| `src/app/cadastro.tsx` | 8 | copy e comentários |
| `src/logic/leituras.ts`, `conselhos.ts`, `alertas.ts`, `avisos.ts` | 12 | texto gerado |
| `(tabs)/jornada.tsx`, `(tabs)/cuidado.tsx`, `perfil.tsx`, `dados.tsx`, `conversa.tsx` | 14 | copy de tela |
| o resto | ~30 | espalhado |

### A regra de substituição

Já existe, e é uma só: `src/logic/formas.ts`.

| no lugar de | use |
|---|---|
| "caneta" | `FORMAS[forma].recipiente` |
| "canetas" | `.plural` |
| "aplicar" | `.verbo` |
| "aplicação" | `.acao` |
| o artigo e a concordância | `oA()`, `umOutro()`, `concordar()` |
| esconder a seção inteira | `.injetavel` |

A forma em uso sai de `formaDe(S)`, que cai no catálogo quando a pessoa
nunca respondeu.

⚠️ **E nem toda menção vira vocabulário.** Três categorias diferentes:

1. **Copy de tela** — troca pelo vocabulário. É a maior parte.
2. **Identificadores de código** — a rota `/caneta`, a rota
   `/caneta-nova`, o campo `S.pen`, a função `canetaAtual`. São dezessete
   ocorrências e **não precisam mudar**: nome interno não é texto de
   tela, e renomear rota quebra link salvo. Se um dia mudarem, é
   refatoração própria, separada da varredura de texto.
3. **Comentários** — precisam ser relidos, não substituídos. Vários
   explicam decisões de desenho em termos de caneta, e o raciocínio muda
   quando a forma passa a variar.

### O que já foi convertido

`aplicacao.tsx`, `aplicacao-ok.tsx`, `caneta-nova.tsx`, `caneta.tsx`,
`cadastro.tsx` e `plano.tsx` — as telas que as fases da forma tocaram. A
regra que segui foi "o que se toca, se conserta", e é a mesma que vale
para quem continuar.
